...
void MeshRenderer::Render() {
    auto component_material=game_object()->GetComponent("Material");
    auto material_=std::dynamic_pointer_cast<Material>(component_material);
    if(material_ == nullptr){
        return;
    }
    auto component_meshFilter=game_object()->GetComponent("MeshFilter");
    auto mesh_filter_=std::dynamic_pointer_cast<MeshFilter>(component_meshFilter);
    ...
    auto current_camera = Camera::currentCamera();
    if((current_camera->cullingMask() & game_object()->Layer()) == 0x00)
    {
        return;
    }
    glm::mat4 projection = current_camera->projection_mat4();
    glm::mat4 view = current_camera->view_mat4();
    auto mvp = projection*view*transform->model();
    ...
    if (vertex_array_object_ == 0) {
        ...
        //上传顶点数据到缓冲区对象
        glBufferData(GL_ARRAY_BUFFER, mesh_filter_->mesh()->vertex_num_ * sizeof(MeshFilter::Vertex),
        ...
        //上传顶点索引数据到缓冲区对象
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh_filter_->mesh()->vertex_index_num_ * sizeof(unsigned short),
    ...
    glUseProgram(gl_program_id);
    {
        ...
        glBindVertexArray(vertex_array_object_);
        {
            glDrawElements(GL_TRIANGLES, mesh_filter_->mesh()->vertex_index_num_, GL_UNSIGNED_SHORT,
