...
Texture2D::Texture2D_ptr CreateTexture(std::string image_file_path) {
    auto texture2d = Texture2D::LoadFromFile(image_file_path);
    return texture2d;
}
...
int main() {
    ...
    init_opengl();
//    auto texture2d = CreateTexture(R"(C:\Users\TJR\Desktop\cpp-game-engine\res\images\input_compressed.cpt)");
//    auto shader = Shader::Find(R"(C:\Users\TJR\Desktop\cpp-game-engine\res\shader\Unlit)");
    Material::Material_ptr material = std::make_shared<Material>();
    material->Parse("material/cude.mat");
    GLuint progam_id = material->shader()->getProgram();
    //获取shader属性ID
    mvp_location = glGetUniformLocation(progam_id, "u_mvp");
    vpos_location = glGetAttribLocation(progam_id, "a_pos");
    vcol_location = glGetAttribLocation(progam_id, "a_color");
    a_uv_location = glGetAttribLocation(progam_id, "a_uv");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUseProgram(progam_id);
        {
            ...
            {
                u_diffuse_texture_location = glGetUniformLocation(progam_id, texture.first.c_str());
                //激活纹理单元0
                glActiveTexture(GL_TEXTURE0+i);
                //将加载的图片纹理句柄，绑定到纹理单元0的Texture2D上。
                glBindTexture(GL_TEXTURE_2D, texture.second->gl_texture_id_);
                //设置Shader程序从纹理单元0读取颜色数据
                glUniform1i(u_diffuse_texture_location, i);
                i++;
            }
